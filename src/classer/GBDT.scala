...
object GBDT {
  ...
  def PreProccess1(path: String): DataFrame = {
    ...
    val sparkSession = SparkSession.builder().master("spark://192.168.0.108:7077").appName("GBTClassifier").getOrCreate()
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Make predictions.
    val predictions = gbt.fit(trainingData).transform(testData)
